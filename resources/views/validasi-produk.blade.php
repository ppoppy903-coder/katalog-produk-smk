        ...
        <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr>
                    <td class="p-4">
                        @php
                            $fotos = json_decode($item->foto_produk);
                        @endphp
                        <img src="{{ asset('storage/'.($fotos[0] ?? '')) }}" class="w-16 h-16 object-cover rounded-lg">
                    </td>
                    <td class="p-4 font-bold text-[#0F2857]">{{ $item->nama_produk }}</td>
                    <td class="p-4">{{ $item->user->name ?? 'Siswa' }}</td>
                    
                    <td class="p-4">
                        <div class="font-semibold text-[#0F2857]">{{ $item->nama_sekolah ?? '-' }}</div>
                        <div class="text-xs text-slate-400">{{ $item->jurusan ?? '-' }}</div>
                    </td>
