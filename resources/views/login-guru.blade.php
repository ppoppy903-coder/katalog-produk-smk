...
<body class="min-h-screen flex flex-col lg:flex-row bg-slate-50 text-slate-800 overflow-x-hidden">
    ...
    <div class="w-full lg:w-7/12 flex flex-col justify-center py-12 px-4 sm:px-6 lg:px-20 min-h-screen">
        <div class="max-w-md w-full mx-auto bg-white p-8 sm:p-10 rounded-3xl shadow-xl shadow-slate-200/50">
            <div class="mb-8">
                <h2 class="text-2xl font-extrabold text-[#0A193F] mb-2">Masuk sebagai Guru</h2>
                <p class="text-slate-500 text-sm">Silakan masukkan NPSN dan kata sandi Anda untuk mengakses dashboard guru pembimbing.</p>
            </div>
            ...
            <form action="{{ route('login.guru.proses') }}" method="POST" class="space-y-5">
                @csrf
                
                {{-- Input NPSN --}}
                <div>
                    <label class="block text-[10px] font-bold text-[#0A193F] uppercase mb-1.5 ml-1">NPSN</label>
                    <input type="text" name="npsn" value="{{ old('npsn') }}" placeholder="Masukkan 8 digit NPSN sekolah" required 
                           class="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm focus:border-[#0A193F] focus:ring-1 focus:ring-[#0A193F] outline-none transition-all">
                </div>
            ...
            <p class="text-center text-sm text-slate-500 mt-8">
                Belum punya akun? <a href="{{ route('daftar') }}" class="font-bold text-[#0A193F] hover:underline">Daftar Sekarang</a>
            </p>
        </div>
    </div>
</body>
